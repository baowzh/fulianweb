...
class mail_tag {
	...
	public function __construct() {
		$this->mail_db = pc_base::load_model('mail_model');
	...
	public function lists($data) {
		...
		$siteid = get_siteid();
  		...
  		$r = $this->mail_db->select($sql, '*', $data['limit'], ' '.$data['order']);
		$pages = $this->db->pages;
		return new_html_special_chars($r);
	}
		/**
	...
	if ($pass!='' and $mail_id!=''){
	$sql = array('mailid'=>$mail_id,'pass'=>$pass,'siteid'=>$siteid);
	}elseif($mail_id!==''){
	$sql = array('mailid'=>$mail_id,'siteid'=>$siteid);
	}elseif($bh!=''){
	$sql = array('bh'=>$bh,'siteid'=>$siteid);
	}
  		$r = $this->mail_db->select($sql, '*', $data['limit'], 'addtime '.$data['order']);
		return new_html_special_chars($r);
	}
	...
	public function get_typelist($siteid='1', $value = '', $id = '') {
   			$data = $arr = array();
			$data = $this->type_db->select(array('module'=>'mail', 'siteid'=>$siteid));
