...
class dynaform extends admin{
	...
	public function zmjt(){
		$this->db = pc_base::load_model('dynaform1_model');
		$siteid = SITEID;
		$page = max(intval($_GET['page']),1);
		$infos = $this->db->listinfo(array('itype'=>'zmjt'),'',$page,30);
		include $this->admin_tpl('zmjt');
	}
		...
		$this->db = pc_base::load_model('dynaform1_model');
  		if((!isset($_GET['mailid']) || empty($_GET['mailid'])) && (!isset($_POST['mailid']) || empty($_POST['mailid']))) {
			...
			if(is_array($_POST['mailid'])){
				...
				showmessage(L('operation_success'),'?m=dynaform&c=dynaform&a=zmjt');
			}else{
				$mailid = intval($_GET['mailid']);
				if($mailid < 1) return false;
				//删除友情链接
				...
				if($result){
					...
					showmessage(L("operation_failure"),'?m=dynaform&c=dynaform&a=zmjt');
				}
			}
			showmessage(L('operation_success'), HTTP_REFERER);
		}
