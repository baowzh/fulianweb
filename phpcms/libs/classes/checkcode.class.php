...
class checkcode {
	 ...
	 * 生成文字
	 */
	protected function creat_font() {
		$xi = $this->width / $this->code_len;
		$this->x_start=3;
		for($i = 0; $i < $this->code_len; $i ++) {
			ImageTTFText ( $image = $this->img, $size = $this->font_size, $angle = rand ( - 30, 30 ), $x = $xi * $i + 7, $y = $this->height / 1.4, $color = $this->font_color, $fontfile = $this->font, $text = $this->code [$i] );
			if ($i == 0)
				$this->x_start = $x * $i + 5;
		}
	}
